...
def clean_site_id(site_text):
    """Extract numeric USGS station ID from pasted text."""
...
def safe_name(text, max_len=31):
    """Create a safe filename or Excel sheet name."""
    text = str(text)
...
def normalize_blank(value):
    """Convert NaN and None values to clean strings."""
    if value is None:
        return ""
...
def parse_rdb(text):
    ...
    if len(df) > 0:
        ...
        type_like_count = sum(
            bool(re.match(r"^\d+[snfd]$", item.strip()))
...
def make_excel_safe(df):
    """Remove timezone-aware datetime values before Excel export."""
...
def make_unique_sheet_name(base_name, used_sheet_names):
    """Create unique Excel sheet names."""
...
def classify_dashboard_groups(row):
    """Classify USGS series into dashboard-like groups."""
...
# ============================================================
# USGS inventory and metadata
# ============================================================

def discover_available_series(station_ids):
    """Get exact available period-of-record data series."""
    params = {
        "format": "rdb",
    ...
    if "count_nu" in inventory_df.columns:
        inventory_df["count_nu_numeric"] = pd.to_numeric(
            inventory_df["count_nu"],
            errors="coerce",
        )

    if "begin_date" in inventory_df.columns:
        inventory_df["begin_date_parsed"] = pd.to_datetime(
            inventory_df["begin_date"],
            errors="coerce",
        )

    if "end_date" in inventory_df.columns:
        inventory_df["end_date_parsed"] = pd.to_datetime(
            inventory_df["end_date"],
            errors="coerce",
        )
...
def select_requested_series(inventory_df, requested_groups):
    """Filter inventory by selected dashboard groups."""
    if inventory_df.empty:
        return inventory_df
    ...
    if "ALL" not in requested_groups:
        ...
        selected_df = selected_df[
            selected_df["dashboard_groups"].apply(row_matches)
        ].copy()

    selected_df["data_type_cd_lower"] = selected_df["data_type_cd"].astype(str).str.lower()
    selected_df["download_supported"] = selected_df["data_type_cd_lower"].isin(
        SUPPORTED_DOWNLOAD_DATA_TYPES
    )
...
def get_location_details(station_ids):
    """Get expanded location metadata."""
    params = {
        "format": "rdb",
        "sites": ",".join(station_ids),
        "siteOutput": "expanded",
    ...
    response = request_get(ENDPOINTS["site"], params=params)
    location_df = parse_rdb(response.text)

    return location_df


# ============================================================
# WaterML parser and download
# ============================================================

def get_first_code_value(code_list):
    """Extract code value from USGS WaterML JSON code lists."""
...
def parse_waterml_json(json_data, requested_metadata):
    """Parse USGS WaterML JSON response."""
    records = []
    ...
    for ts in time_series_list:
        ...
        if isinstance(variable.get("unit"), dict):
            unit = (
                variable["unit"].get("unitAbbreviation", "")
                or variable["unit"].get("unitCode", "")
            )
...
def build_download_request(row, start_date, end_date):
    """Build endpoint and request parameters from one selected row."""
...
def download_one_exact_series(row, start_date, end_date):
    """Download one exact time series."""
...
def run_downloads(selected_df, output_dir, start_date, end_date):
    """Download all selected supported time series and save Excel files."""
    ...
    download_df = selected_df[
        selected_df["data_type_cd_lower"].isin(SUPPORTED_DOWNLOAD_DATA_TYPES)
    ].copy()
    ...
    for site_no, site_rows in download_df.groupby("site_no"):
        ...
        with pd.ExcelWriter(site_file, engine="openpyxl") as writer:
            ...
            make_excel_safe(site_rows).to_excel(
                writer,
            ...
            for _, row in site_rows.iterrows():
                ...
                ts_df, used_url, status = download_one_exact_series(
                    row=row,
                ...
                if ts_df.empty:
                    ...
                    make_excel_safe(no_data_df).to_excel(
                        writer,
                    ...
                    minimal_sheet = make_unique_sheet_name(
                        f"min_{base_sheet_name}",
                        used_sheet_names,
                    )

                    make_excel_safe(ts_df[minimal_cols]).to_excel(
                        writer,
                        sheet_name=minimal_sheet,
                        index=False,
                    )
...
if "selected_df" in st.session_state:
    ...
    unsupported_df = selected_df[
        selected_df["download_supported"] == False
    ] if "download_supported" in selected_df.columns else pd.DataFrame()
